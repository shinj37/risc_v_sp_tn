...
always_comb begin
    ...
    // If instruction in EX is a load, and it writes to a register that
    // the instruction in ID needs to read, we must stall
    if (ex_mem_read && ex_reg_write && 
        ((ex_rd == id_rs1) || (ex_rd == id_rs2)) &&
        (ex_rd != 5'b0)) begin
        stall = 1'b1;  // Stall IF and ID stages for one cycle
    end
